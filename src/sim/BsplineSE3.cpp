...
void BsplineSE3::feed_trajectory(std::vector<Eigen::VectorXd> traj_points) {
  ...
  dt = (dt < 0.05) ? 0.05 : dt;
  printf(CYAN "[B-SPLINE]: control point dt = %.3f (original dt of %.3f)\n", dt, sumdt / (traj_points.size() - 1));
  ...
  printf(CYAN "[B-SPLINE]: trajectory start time = %.6f\n", timestamp_min);
  printf(CYAN "[B-SPLINE]: trajectory end time = %.6f\n", timestamp_max);
  ...
  printf(CYAN "[B-SPLINE]: start trajectory time of %.6f\n", timestamp_start);
}
